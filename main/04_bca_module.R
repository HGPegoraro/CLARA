...
bca_server <- function(id, global_excel_format_reactive) {
  moduleServer(id, function(input, output, session) {
    ...
    output$standards_config_ui <- renderUI({
      ...
      if (nrow(config_df) == 0) return(p("Click 'Add Point' to start."))
      ...
      tagList(
        lapply(1:nrow(config_df), function(i) {
          ...
          fluidRow(class="std-point-row",
                   ...
                   column(5, actionButton(ns(paste0("assign_", point_id)), "Assign Wells", icon = icon("arrow-left"), width = "100%")),
                   column(3, actionButton(ns(paste0("remove_", point_id)), NULL, icon = icon("trash"), class="btn-danger", width="100%")),
                   column(12, tags$p(
                     tags$small(paste("Wellss:", paste(row$wells[[1]], collapse=", ") %||% "None"))
                   ))
          )
        })
      )
    ...
    observe({
      ...
      lapply(config_df$id, function(point_id) {
        observeEvent(input[[paste0("assign_", point_id)]], {
          ...
          showNotification(paste(length(selected), "wells attributed"), type="message")
      ...
      lapply(config_df$id, function(point_id) {
        observeEvent(input[[paste0("remove_", point_id)]], {
          ...
          showNotification("Removed point from curve", type="message")
        }, ignoreInit = TRUE, ignoreNULL = TRUE)
      })
    })
    ...
    observeEvent(input$clear_ignored_stds, {
      ...
      if (!is.null(current_ignored_list[[fname]]) && length(current_ignored_list[[fname]]) > 0) {
        current_ignored_list[[fname]] <- character(0)
        ignored_standard_wells_all_plates(current_ignored_list)
        showNotification(paste("All ignored standards for plate", fname, "were restored."), type="message")
      } else {
        showNotification("No standard ignored to restore on this plate.", type="info")
      }
    })
    ...
    generate_std_curve_logic <- eventReactive({
      ...
      if(nrow(std_curve_data_for_fit) < 2) {
        showNotification("Insufficient standard points (minimum 2) to generate the curve.", type="error")
        output$std_curve_plot <- renderPlot({ plot.new(); title("Insufficient standard points") })
        output$std_curve_summary <- renderText("Error: Insufficient standard points.")
        all_data$files[[fname_logic]]$standard_curve_fit <- NULL
        all_data$files[[fname_logic]]$standard_curve_data_points <- NULL
        return()
      }

      fit <- lm(AbsorbanciaMedia ~ Concentration, data = std_curve_data_for_fit)
      ...
      output$std_curve_summary <- renderPrint({
        ...
        cat(sprintf("Equation: Absorbance = %.4f * Concentration + %.4f\n", coefs[2], coefs[1]))
        cat(sprintf("Number of standard points used in the curve: %d\n", nrow(std_curve_data_for_fit)))
      })
    ...
    interpolate_samples_logic <- function(fname) {
      ...
      if (is.null(coefs) || !("Concentration" %in% names(coefs))) return(NULL)
      slope <- coefs["Concentration"]
      ...
      samples_grouped <- samples_on_plate_individual_wells %>%
        ...
        mutate(
          ...
          Concentration_Interpolada = Mean_Abs_Corrigida / slope
        ) %>%
        mutate(
          Concentration_Interpolada = ifelse(is.na(Concentration_Interpolada) | Concentration_Interpolada < 0, 0, round(Concentration_Interpolada, 3)),
      ...
      if (fname %in% names(all_data$files)) {
        ...
        if(!is.null(interpolated_df) && nrow(interpolated_df) > 0){
          output$interpolated_values_table <- renderDT({
            datatable(interpolated_df[, c("Amostra", "Mean_Abs_Raw", "SD_Abs_Raw", "N_Replicas", "Mean_Abs_Corrigida", "Concentration_Interpolada", "Poços_Replicas")],
                      options = list(pageLength = 10, scrollX = TRUE), rownames = FALSE,
                      colnames = c("Sample", "Average Absorbance (Raw)", "SD Abs (Raw)", "Nº Replicas", "Average Absorbance (Corrected)", "Interpolated Conc.  (µg/µL)", "Wells"))
          }, server = FALSE)
        } else {
          output$interpolated_values_table <- renderDT({
            datatable(data.frame(Mensagem="No Samples to Interpolate."), options=list(dom='t', searching = FALSE, paging=FALSE), rownames=FALSE)
          }, server = FALSE)
        }
      } else {
    ...
    observeEvent(input$files, {
      ...
      if (is.null(reader_function)) {
        showNotification(paste("Invalid reader format:", current_global_format_id), type = "error")
        return()
      }
      ...
      withProgress(message = "Loading Files...", value = 0, {
        for (i in seq_len(nrow(input$files))) {
          ...
          if (is.null(plate_values_named_vector)) {
            showNotification(paste("Failure to process:", original_fname), type = "error")
            next
          }
    ...
    observeEvent(input$add_sample, {
      ...
      if (trimws(input$sample_name) == "") {
        showNotification("Enter a valid name for the sample.", type = "error"); return()
      }
      selected_wells_current <- input$selected_wells
      if (is.null(selected_wells_current) || length(selected_wells_current) == 0) {
        showNotification("No Well Selected", type = "warning"); return()
      }
      ...
      if (length(actual_selected_wells) == 0) {
        showNotification("No valid (non-standard) well selected.", type = "warning"); return()
      }
    ...
    observeEvent(input$remove_from_sample, {
      ...
      showNotification("Attribution removed.", type = "message")
    })
      ...
      for (r_idx in 1:nrow(plate_layout_matrix)) {
        ...
        for (c_idx in 1:ncol(plate_layout_matrix)) {
          ...
          if(!is_standard && !is.null(assigned_samples_df) && nrow(assigned_samples_df) > 0){
            ...
            conc_display <- if (length(conc_value) == 1) sprintf("%.3f", conc_value) else "Standard"
    ...
    observeEvent(input$load_state, {
      ...
      if (length(placas_carregadas) > 0) {
        ...
        showNotification("BCA Project loaded.", type = "message")
      } else {
        updateSelectInput(session, "selected_plate", choices = list("No Plate Available" = "NA"), selected = "NA")
        showNotification("RDS FIle Loaded, but with no valid plate.", type = "warning")
      }
    })
  })
}
